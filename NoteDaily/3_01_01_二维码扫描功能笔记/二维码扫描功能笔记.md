...
```js
<div class="button mui-pull-right header-tijiao" onclick="clicked('barcode_scan.html',true,true)">扫码</div>
```
#### 2.按钮和点击函数绑定
**1.1 添加js文件**
```js
<script type="text/javascript" src="../js/scan_common.js"></script>
```
**1.1.1 文件内容**
```js
(function(w){
...
**2.1 添加二维码相机界面文件(barcode_scan.html)**
```html
<!DOCTYPE html>
<html>
	<head>
...
```
**4.1 把获取的二维码内容通过`mui.openWindow()`收集**
...
var self = plus.webview.currentWebview();
				var htmls = self.htmls;
				var scan_code=document.getElementById("scan");
//				scan_code.getAttribute(placeholder:htmls);
				//scan_code.innerHTML=htmls;
				document.getElementById("scan").value=htmls;
...
```html
输出显示代码
<li class="mui-table-view-cell content_li">
						<span class="content_name">二维码：</span>
						<div class="content_msg">
							<input type="hidden" id="place_id" />
							<input type="text" id="scan" readonly="readonly" style="height: 40px;" placeholder="这是二维码读取值" value="" />
						</div>
					</li>
```